...
// Let's say w-1, w-4, w-6 are emergency ready.
seed = seed.replace(/(true, 350, 'verified')/g, 'true, true, 350, \'verified\''); // w-1, w-2, w-4, w-10
seed = seed.replace(/(true, 400, 'verified')/g, 'true, false, 400, \'verified\'');
seed = seed.replace(/(true, 450, 'verified')/g, 'true, true, 450, \'verified\''); // w-6
seed = seed.replace(/(true, 500, 'verified')/g, 'true, false, 500, \'verified\'');
seed = seed.replace(/(true, 300, 'verified')/g, 'true, false, 300, \'verified\'');
...
seed = seed.replace(
  'is_emergency, status', 
  'address, lat, lng, is_emergency, status'
);
